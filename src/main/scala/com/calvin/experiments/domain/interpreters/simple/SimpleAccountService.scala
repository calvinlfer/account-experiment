...
trait SimpleAccountService extends AccountService[MoneyBD, SimpleAccount, ErrorOr] {
  ...
  override def credit(amount: MoneyBD): SimpleAccount => SimpleAccount =
    account => account.copy(balance = account.balance + amount)

  override def debit(amount: MoneyBD): SimpleAccount => ErrorOr[SimpleAccount] =
    account =>
  ...
  override def preAuth(amount: MoneyBD): SimpleAccount => ErrorOr[SimpleAccount] =
    account =>
      if (account.balance < amount) error(s"Not enough money in account")
      else
        success {
          account.copy(balance = account.balance - amount, balanceHeld = account.balanceHeld + amount)
      }

  override def cancel(amount: MoneyBD): SimpleAccount => ErrorOr[SimpleAccount] =
    account =>
