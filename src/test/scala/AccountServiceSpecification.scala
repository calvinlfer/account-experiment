...
class AccountServiceSpecification
    extends Properties("Account Algebra Verification via SimpleAccount")
  ...
  property("""A pre-authorization followed by a capture of $X will cause a reduction to the account of $X and must not
      |affect an existing holding balances""".stripMargin) = forAll(accountGen) { initialAccount =>
    val amount = 10
  ...
  property("""A purchase (acts like a debit) of $X will reduce the balance in the account by $X without touching
      |holding balances""".stripMargin) = forAll(accountGen) { initialAccount =>
    val amount = 10
